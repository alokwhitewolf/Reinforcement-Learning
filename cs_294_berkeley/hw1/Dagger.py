...
def main():
	...
	no_of_nodes = next_greater_power_of_2(obs_data.shape[1])

	model = Sequential()
	model.add(Dense(no_of_nodes, activation='relu', input_shape=(obs_data.shape[1],)))
	...
	saved_from_before = False
	
	for i in range(10):
		print("Dagger loop no, ",i)
		n = obs_data.shape[0]
		...
		#model.save('Dagger_models/' + env_name + '_cloned_model.h5')

		with tf.Session():
			tf_util.initialize()

			if not saved_from_before:
				model.fit(X_train, Y_train, batch_size=64, nb_epoch=30, verbose=1)
				score = model.evaluate(X_test, Y_test, verbose=1)

				model.save('Dagger_models/' + env_name + '_cloned_model.h5')
				saved_from_before = True
			else:
				model = load_model('Dagger_models/' + env_name + '_cloned_model.h5')
				model.fit(X_train, Y_train, batch_size=64, nb_epoch=30, verbose=1)
				score = model.evaluate(X_test, Y_test, verbose=1)
